...
for c_id in ${containers[@]}; do
	...
	if [[ $hostname == "host_lsirigna-1" ]]; then
		...
		docker exec $c_id sh -c " ip a | grep eth1 -A 1 | grep -e inet -e link | sed -e's/  */ /g' | cut -d ' 
' -f 3 | tr '\012' ' ' | awk '{print \"${RED}MAC ADDRESS :\t${END}\" $1 \"${RED}\tIP
 ADDRESS :\t${END}\" $2}'"
...
' -f 3 | tr '\012' ' ' | awk '{print \"${RED}MAC ADDRESS :\t${END}\" $1 \"${RED}\tIP
 ADDRESS :\t${END}\" $2}'"
		...
		docker exec $c_id sh -c " ip a | grep eth0 -A 1 | grep -e inet -e link | sed -e's/  */ /g' | cut -d ' 
' -f 3 | tr '\012' ' ' | awk '{print \"${RED}MAC ADDRESS :\t${END}\" $1 \"${RED}\tIP
 ADDRESS :\t${END}\" $2}'"
		docker exec $c_id sh -c " ip a | grep eth1 -A 1 | grep -e inet -e link | sed -e's/  */ /g' | cut -d ' 
' -f 3 | tr '\012' ' ' | awk '{print \"${RED}MAC ADDRESS :\t${END}\" $1 \"${RED}\tIP
